...
Dedicated host routes may add backend-owned transport metadata. For the Hermes API Server route, ChatRaw derives
...
backend-stored but optional in ChatRaw: when unset, the backend sends no `Authorization` header. Official
...
Hermes can optionally use a backend-saved execution mode. `apiMode: runs` tells the Hermes backend route to
...
existing chat UI, and the backend maps final text/reasoning/error state into the existing response
contract.

Hermes integration QA should cover the plugin lifecycle and the host route together: install and
...
专用宿主路由可以增加后端拥有的传输元数据。Hermes API Server 路由会在缺失或 stale chat 解析完成后，
...
状态。Hermes API Server Key 也由后端保存，但在 ChatRaw 中是可选项：未保存时后端不发送
...
Hermes 可以选择后端保存的执行模式。`apiMode: runs` 表示 Hermes 后端路由在同一个
...
`content`；ChatRaw 继续使用现有聊天 UI，后端只把最终文本、reasoning 和错误状态映射回现有响应
contract。
